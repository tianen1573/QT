...
###### 在Mac下，是否有数据保存在了app下的Contents目录下？

> 在一般情况下，Mac 应用程序的主要数据是不会保存在应用程序包（.app）的 Contents 目录下的。应用程序包是一个文件夹，通常用于存储应用程序的可执行文件、资源文件和其他支持文件。这个包在安装或运行应用程序时是只读的，并且在更新或重新安装应用程序时可能会被替换。
>
> 根据 Mac 应用程序的设计规范，应用程序的数据通常保存在以下位置之一：
>
> 1. 用户主目录（User's Home Directory）：每个用户在 Mac 上都有一个对应的主目录，通常位于 `/Users/username`，其中 `username` 是用户的用户名。在用户主目录下，应用程序可以在特定的子目录中保存数据，例如 `~/Documents`、`~/Library/Application Support`、`~/Library/Preferences` 等。
>
> 2. 全局应用程序支持目录（Global Application Support Directory）：位于 `/Library/Application Support`，用于存储应用程序共享的数据或配置文件。
>
> 3. 缓存目录（Cache Directory）：位于 `~/Library/Caches`，用于存储临时文件或缓存数据。
>
> 4. 配置文件目录（Preferences Directory）：位于 `~/Library/Preferences`，用于存储应用程序的配置文件。
>
> 这些位置提供了持久化存储数据的常见目录，而应用程序包的 Contents 目录则用于存储应用程序的执行文件和资源文件。
>
> 需要注意的是，有些应用程序可能会在特定情况下将数据保存在应用程序包的 Contents 目录下，但这是相对较少见的情况，并且违反了应用程序包的设计原则。通常情况下，应用程序应该将数据保存在适当的用户目录或全局应用程序支持目录中，以确保数据的持久性和访问权限。
>
> 总结而言，一般来说，在 Mac 下应用程序的数据不会保存在应用程序包的 Contents 目录下，而是保存在用户主目录、全局应用程序支持目录、缓存目录或配置文件目录等位置。这样的设计可以确保数据的安全性、持久性和合适的访问权限。

###### Contents运行中的替换问题

> 如果在程序运行过程中，你手动替换了该程序的 Contents 目录，那么程序的运行结果将取决于具体情况。一般来说，这样的操作可能会导致程序出现问题或无法正确运行。
>
> 应用程序的 Contents 目录通常包含应用程序的执行文件和资源文件，这些文件在程序运行时被加载和使用。如果你替换了该目录，可能会导致以下问题：
>
> 1. 文件缺失或版本不匹配：新的 Contents 目录可能缺少某些必要的文件，或者文件版本与程序代码不匹配。这可能导致程序无法找到所需的文件或加载不正确的文件，从而导致功能异常或崩溃。
>
> 2. 文件权限问题：新的 Contents 目录可能具有不正确的文件权限设置，导致应用程序无法读取或写入文件。这可能会导致程序运行时的访问权限错误，进而导致功能异常或数据损坏。
>
> 3. 破坏应用程序签名：应用程序通常会经过数字签名以确保其完整性和来源可信。如果你替换了 Contents 目录，可能会破坏应用程序的签名，从而导致操作系统拒绝运行该程序。
>
> 4. 其他依赖关系问题：应用程序的 Contents 目录可能包含与其他系统组件或库文件的依赖关系。如果你替换了该目录，可能会导致这些依赖关系断裂，从而影响程序的正常运行。
>
> 总之，手动替换应用程序的 Contents 目录可能会引发各种问题，导致程序无法正确运行或出现异常行为。如果需要修改应用程序的内容或资源，建议使用开发者提供的合适的方式进行修改，以确保程序的完整性和稳定性。

###### 手动替换和程序替换的区别

> 1. 操作方式：手动替换是指直接通过操作系统的文件管理器或命令行界面，将一个目录或文件复制到另一个目录，从而替换原有的目录或文件。而通过程序调用 `cp` 命令进行替换，则是在程序中通过系统调用或执行命令的方式，使用 `cp` 命令来完成目录或文件的复制和替换操作。
> 2. 权限和特权：手动替换通常需要具备足够的权限才能访问和修改目标目录或文件，因为这涉及到操作系统的文件系统权限。而通过程序调用 `cp` 命令进行替换，则需要确保程序本身具备足够的权限来执行 `cp` 命令。这可能需要考虑程序运行的用户权限、文件系统权限等因素。
...
> 在Mac下，如果你用未签名的 DMG（磁盘映像文件）的 Contents 目录替换了已经签名的应用程序的 Contents 目录，那么替换后的应用程序可能无法正确运行。
>
> 应用程序在签名之后，签名是用于验证应用程序的完整性和来源可信性的重要标识。签名包含了开发者的证书和相关信息，并与应用程序的内容进行了数字签名。操作系统在运行应用程序时会验证签名，确保应用程序未被篡改或修改。
>
> 当你用未签名的 DMG 的 Contents 目录替换已签名应用程序的 Contents 目录时，会破坏应用程序的签名，导致操作系统无法正确验证应用程序的完整性和可信性。在这种情况下，操作系统可能会拒绝运行被破坏签名的应用程序，或者给出警告提示。
>
> 此外，Mac 的 Gatekeeper 安全功能也可能会对未经签名的应用程序进行额外的限制。未签名的应用程序可能需要用户手动授予权限或修改安全设置才能运行，否则可能会被阻止执行。
>
> 因此，替换已签名应用程序的 Contents 目录可能会导致应用程序无法正确运行或受到系统的限制。如果需要修改已签名应用程序的内容，建议采用合适的方式，如使用开发者提供的更新机制或重新下载经过正确签名的应用程序。这样可以确保应用程序的完整性、可信性和安全性。
>
> ---
>
> 如果你在替换应用程序的 Contents 目录后，应用程序在重启电脑后可以正常运行，可能有几个可能的原因：
>
> 1. 缓存或临时文件：在应用程序运行期间，应用程序可能会在某些位置创建缓存文件或临时文件。当你替换应用程序的 Contents 目录时，这些缓存或临时文件可能仍然存在，并且在重启电脑后，应用程序重新生成或使用了新的 Contents 目录。这可能解决了替换导致的问题，使得应用程序可以正常运行。
>
> 2. 系统重置：重启电脑可能会导致系统状态的重置，包括相关的应用程序和配置。在重启后，操作系统重新加载应用程序及其相关组件，这可能清除了之前替换导致的问题，并使应用程序能够正常运行。
>
> 3. 文件权限修复：在重启时，操作系统可能会自动执行一些文件系统检查和修复操作。这可能包括修复权限问题、恢复损坏的文件或恢复文件的正确状态。如果替换应用程序的 Contents 目录导致了文件权限问题，重启电脑后，这些问题可能会被自动修复，从而使应用程序能够正常运行。
>
> 需要注意的是，尽管重启电脑后应用程序可以正常运行，但替换应用程序的 Contents 目录可能仍然存在一些潜在的问题。这包括应用程序的完整性、更新性和可信性。建议在修改应用程序或替换其内容时，使用开发者提供的合适方式或重新下载经过正确签名的应用程序，以确保应用程序的稳定性和安全性。

用签名过的dmg文件的Contents目录去替换未签名的(Debug)

> 好像没问题

用签名过的dmg文件的Contens目录去替换签过名的

> 好像没问题

用未签名的(Debug/Release)的Contents去替换签名过的app的Contents
...
是否是在运行过程中cp-r指令不能正确的将外部库连接给copy过来？
